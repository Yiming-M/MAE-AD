...
class MSGMSLoss(nn.Module):
    ...
    def forward(self, img1: Tensor, img2: Tensor, as_loss: bool = True) -> Tensor:
        self.prewitt_x = self.prewitt_x.to(img1.device) if self.prewitt_x.device != img1.device else self.prewitt_x
        self.prewitt_y = self.prewitt_y.to(img1.device) if self.prewitt_y.device != img1.device else self.prewitt_y
        ...
        for scale in range(self.num_scales):
            ...
            msgms_map += F.interpolate(gms_map, size=(h, w), mode="bilinear", align_corners=False, antialias=True)

        if as_loss:
            return 1. - (msgms_map / self.num_scales).mean()
        else:
            return torch.mean(msgms_map / self.num_scales, dim=-3, keepdim=True)
    ...
    def _create_prewitt_kernel(self) -> Tuple[Tensor, Tensor]:
        ...
        prewitt_x = prewitt_x.expand(self.in_channels, -1, -1, -1)  # (self.in_channels, 1, 3, 3)
        ...
        prewitt_y = prewitt_y.expand(self.in_channels, -1, -1, -1)  # (self.in_channels, 1, 3, 3)
