...
class MaskedAutoencoderViT(nn.Module):
    ...
    def random_masking_eval(self, x, mask_ratio):
        """
        Perform per-sample random masking by per-sample shuffling.
        Per-sample shuffling is done by argsort random noise.
        x: [N, L, D], sequence
        ...
        # sort noise for each sample
        ids_shuffle = torch.argsort(noise, dim=1)  # ascend: small is keep, large is remove
        ids_restore1 = torch.argsort(ids_shuffle, dim=1)
        ...
        ids_shuffle = torch.cat([ids_shuffle[:, len_keep:], ids_shuffle[:, :len_keep]], dim=1)
        ids_restore2 = torch.argsort(ids_shuffle, dim=1)
        ...
        return x_masked1, mask1, ids_restore1, x_masked2, mask2, ids_restore2
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        # else:
        #     x1, mask1, ids_restore1, x2, mask2, ids_restore2 = self.random_masking_eval(x, mask_ratio)
        #     x1 = self.forward_encoder(x1)
        #     pred1 = self.forward_decoder(x1, ids_restore1)  # [N, L, p*p*3]
        #     pred1 = pred1 * mask1.unsqueeze(-1).expand(-1, -1, pred1.shape[-1])
        #     x2 = self.forward_encoder(x2)
        #     pred2 = self.forward_decoder(x2, ids_restore2)  # [N, L, p*p*3]
        #     pred2 = pred2 * mask2.unsqueeze(-1).expand(-1, -1, pred2.shape[-1])
        #     pred = pred1 + pred2
        #     pred = self.unpatchify(pred)
...
def mae_vit_large_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        ...
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    ckpt = torch.load("mae_visualize_vit_large_ganloss.pth", map_location="cpu")
